...
do
    ...
    sed -n "/^Keyspace: $myKeyspace$/,/^Keyspace/p" $someFile | grep "Column Family:" > $cfName # Get all tables in the Keyspace
...
do
    echo -e "\n$someTable - $myValue"
    echo -e "======================="
    for cfstatsfile in $myDir/*cfstats
    do
        myvalue=$(sed -n "/^Keyspace: $myKeyspace$/,/^Keyspace/p" $cfstatsfile | grep -A25 -w "$someTable" | grep -w "$myValue" )
        echo -e "$cfstatsfile\t$myvalue"
    done
